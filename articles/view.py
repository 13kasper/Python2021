# ДЕКОРАТОР
def add_title(title):  # сюда попадает  = 'Ввод пользовательских данных'
    def wrapper(func):  # сюда попадает функция = wait_user_answer
        def wrap(*args, **kwargs):  # параметры
...
class UserInterface():
    ...
    def wait_user_answer(self):
        ...
        print("1 - создание строки"
              "\n2 - Просмотр статей"
              "\n3 - Просмотр отдульной статьи"
              "\n4 - Удаление статьи"
    ...
    @add_title('Сообщение об ошибке')
    def show_incorrect_answer_eror(self, answer):
